...
public class EditarProduto extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            long formCodigoIdentificador = Long.parseLong(request.getParameter("codigo_identificador"));
            String formNome = request.getParameter("nome");
            String formDescricao = request.getParameter("descricao");
            float formPreco = Float.parseFloat(request.getParameter("preco"));
            
            Produto produto = produtosRepository.findByCodigoIdentificador(formCodigoIdentificador);
            
            produto.setNome(formNome);
            produto.setDescricaoCompleta(formDescricao);
            produto.setPrecoUnitario(formPreco);
            
            produtosRepository.update(produto);
            
            out.printf("<p>Produto %s editado com sucesso!!!</p>", produto.getNome());
            out.println("<a href=\"ListarProduto\">Listar Produto</a>");
            out.println("</body>");
            out.println("</html>");
        }
    }
